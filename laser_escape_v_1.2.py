...
while not KEY(KEY_OK):
    STR("KEYs [Arrow] = 8 Directions",6,30,"red",bg)
    STR("KEY [OK] = Slow Motion",6,55,"green",bg)
    STR("KEY [Backspace] = Pause/UnPause",6,80,"gray",bg)
    STR("KEY [Toolbox] = Rotate",6,105,"white",bg)
    STR("( Press OK to Start )",50,180,"cyan",bg)
F(0,0,322,35,"white")
...
for i in range(150):
    for j in range(110):
      i=RINT(0,321)
      j=RINT(0,222)
      F(i,j,randint(3,9),randint(3,9),(choice([100,200,255]),0,randint(0,50)))
